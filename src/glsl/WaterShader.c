...
void main() {
    ...
    distances[1] = 0.25*pixelsPerMeter;
    distances[2] = 0.5*pixelsPerMeter;
    distances[3] = 1.0*pixelsPerMeter;
    distances[4] = 4.0*pixelsPerMeter;
    ...
    colVecs[3] = vec3(0.1,0.5,0.8);
    colVecs[4] = vec3(0.1,0.2,0.5);
    ...
    if (tex2.a == TEX_WATER) {
        ...
        worldPositionRef = texture2D(Texture8, newTC.xy);
        ...
        for (i = 0; i < maxEntriesM1; i++) {
            ...
            if (
                ...
                finalCol = mix(colVecs[i], colVecs[i+1], lerpVal)*lval;
        ...
        if (tex3.a == 0.0 ) {
            ...
            if (
                ...
                finalCol += finalCol*(rand(TexCoord0.xy*curTime/10000.0)*0.15+0.25)*(1.0-lerpVal)*4.0*mix(0.5,1.0,timeOfDay);
