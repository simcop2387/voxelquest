...
void main()
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		testTex = texture2D(Texture3, TexCoord0.xy + dirModXY[i].xy / bufferDim );
	...
	else
	{
		...
		for (k = 0; k < lightCount; k++)
		{
			...
			if (distance(worldPosition, lightPosWS.xyz) <= lightPosWS.w)
			{
				...
				if (k == 0)
				{
					...
					lightIntensity = mix(0.2, 1.0, timeOfDay);
					lightColorization = mix(0.4, 0.0, timeOfDay);
					lightFlooding = mix(1.0, 0.0, timeOfDay);
				}
				...
				totNonColored += lightDis * resComp2 * (1.0 - lightColorization);

				totColorization += lightColorization * lightDis * clamp(resComp2 + lightFlooding, 0.0, 1.0);
				totLightIntensity += lightIntensity * lightDis * resComp2;

				totLightColor += frontLight * resComp2 * curLightColor * lightDis;

				totDirLight += frontLight * resComp2 * lightDis * float(k != 0);
		...
		else
		{
			...
			resColorTemp = mix(
											 ...
											 resColorTemp,
											 min(frontLightTot + 0.5, 1.0) * resCompTot
										 ) + totLightColorNoShadow;//mix(totLightColorNoShadow,vec3(0.0),timeOfDay);
			...
			resColorTemp = mix(
											 ...
											 mix(
												 ...
												 clamp(totLightColor, 0.0, 1.0) * (lightRes + clamp(totLightDis, 0.0, 1.0) ) + totLightColorNoShadow, //*totLightIntensity
												 totHits2
											 ),
