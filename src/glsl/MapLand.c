...
vec3 getTerVal(vec3 pos, float camParam, bool justLand) {
	...
	if (camDis1 > 0.0) {
		//getTexCubic
		samp1 = getTexCubic(Texture13, pos*(vec3(0.25,0.25,0.25*0.5)*0.5), voroSize);
		//samp1 = trilin(Texture13, pos*vec3(32.0,32.0,8.0)/(cellsPerWorld), vec3(voroSize),1.0/vec3(voroSize));
		//globTexTap += 1.0;
		
	}
	// if (camDis5 > 0.0) {
	// 	bumpDet = bilin(Texture2, newTC.xy*16.0 + 0.74, mp2, mp).r*camDis5;
	...
	if (camDis2 > 0.0) {
		samp2 = getTexCubic(Texture13, pos*(1.0+randVal2*0.00625)*0.25, voroSize);
		//globTexTap += 1.0;
	}
	...
	// if (camDis4 > 0.0) {
	// 	bumpDet += (
	// 		sin(bilin(Texture2, newTC.xy*256.0 + 0.3, mp2, mp).r*8.0) + 1.0	
	// 	)*1.0*camDis4;
	...
	if (camDis3 > 0.0) {
		samp3 = getTexCubic(Texture13, pos*(12.0+randVal3*0.025)*0.125, voroSize);
		//globTexTap += 1.0;
	...
	if (camDis2 > 0.0) {
		res = opD(res,pow(1.0-samp2.r,4.0)*32.0*camDis2);
	}
