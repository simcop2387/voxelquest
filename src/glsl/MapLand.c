...
vec3 getTerVal(vec3 pos, float camParam, bool justLand) {
	...
	float camDis5 = pow(clamp(1.0-camDis*8.0,0.0,1.0),0.25);
	...
	if (camDis1 > 0.0) {
		//getTexCubic
		samp1 = getTexLin(Texture13, pos*(vec3(0.25,0.25,0.25*0.25)*1.0), voroSize);
	...
	if (camDis4 > 0.0) {
		bumpDet = bilin(Texture2, newTC.xy*1024.0 + 0.3, mp2, mp).r*4.0*camDis4;
		
	}
	if (camDis5 > 0.0) {
		bumpDet += bilin(Texture2, newTC.xy*64.0 + 0.74, mp2, mp).r*64.0*camDis5;
	...
	if (camDis2 > 0.0) {
		...
		//res = opD(res,pow(1.0-samp2.r,4.0)*8.0*thMod*camDis2);
		res = opD(res,pow(1.0-samp2.r,4.0)*2.0*thMod*camDis2);
	}
